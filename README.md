...
| 港股 | 雪球 HK API | 不需要登录凭证 (v1.1+ 起自动获取临时会话凭证) |
...
先取消隐藏对应市场的「抓取诊断」sheet,查看状态、数据源、字段命中、单位、汇率和备注。港股或中概美股若批量失败,常见原因是临时会话凭证自动获取被拦截(例如本机 PowerShell 受限、网络限流或防火墙拦截外网请求),稍后重试或更换网络环境通常即可恢复。
...
- **v1.1.x (Phase 5a)** — 雪球数据源改为自动获取临时会话凭证,不再需要手工维护 cookie;样本池 `E5` 已弃用并从布局中移除。
- **v1.1** — 新增台股 (TW) 市场支持,数据源接入 5 张相关报表;跨市场指标表覆盖 A 股 / 美股 / 港股 / 韩股 / 台股 5 个市场。
