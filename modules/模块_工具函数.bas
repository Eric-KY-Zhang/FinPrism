...
'   Row 13: 数据表头
'   Row 14+: 股票数据
'   列    : A:B=A股, D:E=美股, G:H=港股, J:K=韩股, N:O=台股
...
Public Const POOL_TW_CODE_COL As Long = 14
Public Const POOL_TW_NAME_COL As Long = 15
Public Const POOL_LAST_COL As Long = 16       ' 新布局视觉区到 P 列
...
Public g_diagnosticSheetName As String      ' 诊断目标 sheet; 默认美股_抓取诊断, 港股/韩股/台股入口切到对应诊断 sheet
...
'   Phase 5a 起改走 PowerShell + /hq 匿名 warmup, session cookie 由
'   HttpClient.UseCookies 自动管理。strCookie 参数保留以维持 20+ 调用方
'   签名兼容,实际不再使用;样本池!E5 可以留空。
Public Function XueqiuHttpGet(ByVal strUrl As String, ByVal strCookie As String) As String
    XueqiuHttpGet = FetchViaPowerShell(strUrl, True)
End Function
...
'   curCode   : "RMB" / "CNY" / "USD" / "HKD" / "KRW" / "TWD" (其他返 0)
...
Public Function GetFxRateStatus(ByVal curCode As String, ByVal periodEnd As String, _
    ...
    If rowIdx > 0 Then
        ...
        If FxValueIsPositive(v) Then
            ...
            Exit Function
        Else
            ' 该报告期行已存在但目标币种列为空/无效时仍允许补抓。
            ' 典型场景: 旧工作簿已有 USD/HKD/KRW 汇率行, 后续接入台股才新增 TWD 列。
        End If
    End If

    ' Period row missing or target currency columns missing → 拉数.
    ' EnsureFxRateCached 只填缺失/无效的目标币种列, 不覆盖用户已有的有效 override。
    Dim ok As Boolean
    On Error Resume Next
...
Public Function LookupFxColForCurrency(ByVal curCode As String, ByVal useEop As Boolean) As Long
    ...
    Select Case c
        ...
        Case "TWD"
            LookupFxColForCurrency = IIf(useEop, 8, 9)
        Case Else
...
Public Function GetTtlHoursForSource(ByVal sourceName As String) As Long
    Select Case UCase$(Trim$(sourceName))
        ...
        Case "EDGAR", "EDGAR_COMPANYFACTS", "STOCKANALYSIS", "STOCKANALYSIS_KR", "STOCKANALYSIS_US", "FINMIND", "FX_KLINE"
...
Public Function FinMindHttpGet(ByVal strUrl As String) As String
    Dim objWinHttp As Object, arrByte() As Byte
    Set objWinHttp = CreateObject("WinHttp.WinHttpRequest.5.1")
    With objWinHttp
        .Open "GET", strUrl, False
        ...
        .SetRequestHeader "Accept-Language", "zh-TW,zh;q=0.9,en;q=0.8"
        .SetRequestHeader "Accept-Encoding", "identity"
        .SetTimeouts 10000, 10000, 30000, 60000
        .Send
        .WaitForResponse 60
        If .Status < 200 Or .Status >= 300 Then
            Err.Raise vbObjectError + 748, "FinMindHttpGet", _
                "HTTP " & .Status & " for " & strUrl
        End If
        arrByte = .ResponseBody
    End With
    FinMindHttpGet = ByteToStr(arrByte, "utf-8")
End Function
...
Public Sub WriteWideTable(ByVal ws As Worksheet, _
    ...
    If ws.Name = "A股_指标表" Or ws.Name = "美股_指标表" Or _
       ws.Name = "港股_指标表" Or ws.Name = "韩股_指标表" Or _
       ws.Name = "台股_指标表" Then metaCols = 3
...
Public Function UnitDescriptionForMarket(ByVal sheetName As String) As String
    Select Case True
        ...
        Case InStr(sheetName, "台股") > 0
            UnitDescriptionForMarket = "百万 TWD (FinMind)"
        Case Else
...
' --------- 生成只含 18 个标准指标的指标表 ---------
'   market: "A" / "US" / "HK" / "KR" / "TW"; 从对应资产负债表复制公司/报告期表头, 再填标准指标公式
Public Sub BuildStandardIndicatorSheet(ByVal market As String)
    ...
    ElseIf marketKey = "TW" Then
        targetSheet = "台股_指标表"
        sourceSheet = "台股_资产负债表"
    Else
        marketKey = "A"
        targetSheet = "A股_指标表"
...
' --------- Phase 4g Step 2: 把 5 张分市场指标表合并展示到『跨市场_指标表』 ---------
...
Public Sub BuildCrossMarketIndicatorSheet()
    ...
    Dim markets As Variant: markets = Array("A", "US", "HK", "KR", "TW")
    Dim m As Variant
    ...
    commentText = "跨市场指标合表 (公司数=" & collCompanies.Count & ")" & vbCrLf & _
                  "数据源: 5 张分市场指标表 (引用公式, 自动同步)" & vbCrLf & _
...
Private Function CheckBalanceSheetEquation(ByRef checkedCells As Long) As Long
    ...
    For Each sheetName In Array("A股_资产负债表", "美股_资产负债表", "港股_资产负债表", "韩股_资产负债表", "台股_资产负债表")
...
Private Function CountFxMissingDiagnostics() As Long
    Dim sheetName As Variant
    For Each sheetName In Array("美股_抓取诊断", "港股_抓取诊断", "韩股_抓取诊断", "台股_抓取诊断")
        If Not WorksheetExists(CStr(sheetName)) Then GoTo NextSheet
        Dim ws As Worksheet: Set ws = ThisWorkbook.Sheets(CStr(sheetName))
        Dim r As Long, lastRow As Long
...
Private Function CheckKeyFieldsPresence(ByRef checkedFields As Long) As Long
    ...
    For Each marketPrefix In Array("A股", "美股", "港股", "韩股", "台股")
...
Private Function MarketIndicatorSheetName(ByVal market As String) As String
    Select Case UCase$(Trim$(market))
        ...
        Case "TW": MarketIndicatorSheetName = "台股_指标表"
    End Select
End Function
...
' --------- 指标表追加标准指标层 ---------
'   market: "A" / "US" / "HK" / "KR" / "TW"
'   依赖对应市场的资产负债表、利润表已经生成; 缺字段时公式留空
Public Sub AppendStandardIndicators(ByVal ws As Worksheet, ByVal market As String)
    Dim marketKey As String: marketKey = UCase$(Trim$(market))
    If marketKey <> "A" And marketKey <> "US" And marketKey <> "HK" And marketKey <> "KR" And marketKey <> "TW" Then Exit Sub
...
Private Function StandardRowMap(ByVal marketKey As String) As Object
    ...
    ElseIf marketKey = "HK" Then
        ...
        AddStandardRow m, "CASH", "港股_现金流量表", Array("Cash at end of period")
    ElseIf marketKey = "KR" Then
    ...
    ElseIf marketKey = "TW" Then
        AddStandardRow m, "REV", "台股_利润表", Array("Revenue")
        AddStandardRow m, "COGS", "台股_利润表", Array("Cost of goods & services sold")
        AddStandardRow m, "GP", "台股_利润表", Array("Gross profit")
        AddStandardRow m, "OPEX", "台股_利润表", Array("Total operating expenses")
        AddStandardRow m, "NI", "台股_利润表", Array("Net income")
        AddStandardRow m, "PNI", "台股_利润表", Array("Net income")
        AddStandardRow m, "TA", "台股_资产负债表", Array("Total assets")
        AddStandardRow m, "TL", "台股_资产负债表", Array("Total liabilities")
        AddStandardRow m, "EQ", "台股_资产负债表", Array("Total stockholders' equity", "Total equity")
        AddStandardRow m, "CA", "台股_资产负债表", Array("Total current assets")
        AddStandardRow m, "CL", "台股_资产负债表", Array("Total current liabilities")
        AddStandardRow m, "INV", "台股_资产负债表", Array("Inventory")
        AddStandardRow m, "AR", "台股_资产负债表", Array("Accounts receivable, net")
        AddStandardRow m, "AP", "台股_资产负债表", Array("Accounts payable")
        AddStandardRow m, "CASH", "台股_资产负债表", Array("Cash & equivalents")
    Else
...
Private Function StandardIndicatorFormula(ByVal metricKey As String, ByVal marketKey As String, _
    ...
    Dim bsSheet As String, isSheet As String, cfSheet As String
    If marketKey = "US" Then
        bsSheet = "美股_资产负债表"
        isSheet = "美股_利润表"
        cfSheet = "美股_现金流量表"
    ...
    Dim cfCol As Long: cfCol = FindStandardStatementColumn(cfSheet, companyHeader, periodText)
    ...
    Dim cashCol As Long: cashCol = bsCol
    If rowMap.Exists("CASH") Then
        Dim cashMeta As Variant: cashMeta = rowMap.Item("CASH")
        If CStr(cashMeta(0)) = cfSheet Then cashCol = cfCol
    End If
    Dim cashRef As String: cashRef = StandardRef(rowMap, "CASH", cashCol)
    ...
    Select Case metricKey
        ...
        Case "OER"
            Dim opex As String
            If marketKey = "US" Or marketKey = "KR" Or marketKey = "TW" Then
...
Private Function StandardRawDumpRow(ByVal sheetName As String, _
                                    ByVal displayRow As Long, _
                                    ByVal colNum As Long) As Long
    StandardRawDumpRow = displayRow
    On Error Resume Next
    Dim ws As Worksheet
    Set ws = ThisWorkbook.Sheets(sheetName)
    If ws Is Nothing Then GoTo CleanExit
    If displayRow < 3 Or colNum <= 0 Then GoTo CleanExit

    ' WriteWideTable keeps original-currency values in a hidden dump block.
    ' Indicator formulas must use that block so ratios stay currency-invariant
    ' when the visible statement sheets are toggled to unified RMB display.
    Dim rawDataStartRow As Long: rawDataStartRow = 202
    If ws.Rows(200).Hidden Then
        Dim rawRow As Long: rawRow = rawDataStartRow + displayRow - 3
        If rawRow <= ws.Rows.Count Then
            StandardRawDumpRow = rawRow
        End If
    End If
